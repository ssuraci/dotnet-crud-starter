...
public class TeacherRepository: BaseRepository<Teacher, int>
{
    ...
    public TeacherRepository(ILogger<BaseRepository<Teacher, int>> logger, TeacherDbContext dbContext) : base(logger, dbContext)
    {
        AddIncludeGraph("WithSchool",  x => x.School);
        AddIncludeGraph("WithSchoolAndCourses", x => new { x.School, x.Courses });
